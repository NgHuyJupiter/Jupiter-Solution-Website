    ...
    <div className="flex flex-col w-full">
      ...
      <Section variant="default" className="border-b border-[var(--color-border-default)] bg-editorial-grid relative">
        <Container className="relative z-10">
          ...
          {/* Ratio cards — structured rows, NOT icon cards */}
          <div className="space-y-3">
            {ratios.map((ratio, index) => (
              <div
                key={ratio.title}
                className="group relative bg-white border border-[var(--color-border-default)] hover:border-brand-200 rounded-[var(--radius-lg)] p-5 grid grid-cols-1 sm:grid-cols-[80px_180px_1fr] gap-4 items-center hover-lift opacity-0 animate-fade-up"
                style={{ animationDelay: `${(index + 1) * 60}ms` }}
              >
                {/* Percentage — dominant number */}
                <span className="text-3xl font-extrabold text-brand-600 leading-none tabular-nums group-hover:scale-105 origin-left transition-transform duration-[var(--motion-base)]">
                  {ratio.percent}
                </span>

                {/* Title */}
                <h4 className="text-sm font-semibold text-text-primary group-hover:text-brand-700 transition-colors duration-[var(--motion-base)] leading-snug">
                  {ratio.title}
                </h4>
                ...
                {/* Corner mark */}
                <span className="absolute top-2.5 right-2.5 font-mono text-[8px] text-slate-300 group-hover:text-brand-300 transition-colors duration-[var(--motion-base)] select-none" aria-hidden="true">+</span>
              </div>
            ))}
          </div>
        </Container>
        ...
        <Container>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {certs.map((cert, index) => (
              <div
                key={cert.code}
                className="group bg-white border border-[var(--color-border-default)] hover:border-brand-200 rounded-[var(--radius-lg)] p-5 hover-lift opacity-0 animate-fade-up"
                style={{ animationDelay: `${(index + 1) * 55}ms` }}
              >
                {/* Cert code badge */}
                <span className="font-mono text-[10px] font-bold text-slate-500 bg-slate-100 group-hover:bg-brand-50 group-hover:text-brand-700 transition-colors duration-[var(--motion-base)] px-2 py-0.5 rounded-[var(--radius-xs)] uppercase tracking-widest inline-block mb-3">
                  {cert.code}
                </span>

                {/* Cert title */}
                <h4 className="text-xs font-semibold text-text-primary leading-snug mb-2 group-hover:text-brand-700 transition-colors duration-[var(--motion-base)]">
                  {cert.title}
                </h4>
      ...
      <Section variant="default">
        <Container className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-5 bg-[var(--color-surface-alt)] border border-[var(--color-border-default)] rounded-[var(--radius-lg)] overflow-hidden">
